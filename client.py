...
def run_payment_bot():
    ...
    print_header("Payment Bot - Auto Generate & Pay")
    
    client = FulmineSparkClient()
    
    prompt = input("Enter prompt for image generation (or press Enter for default): ").strip()
    if not prompt:
        prompt = "A beautiful sunset over mountains"
    ...
    print(f"✅ Image generated, invoice created")
    print(f"   Amount: {invoice['amount_sats']} sats (${invoice['price_usd']:.4f})")
    print(f"   Payment Hash: {payment_hash[:16]}...\n")
    
    # Step 2: Pay invoice
    print("=" * 80)
    print("STEP 2: Paying invoice")
    print("=" * 80)
    print(f"Submitting payment of {invoice['amount_sats']} sats...\n")
    
    pay_result = client.pay_invoice(payment_request)
    
    if "error" in pay_result:
        print(f"❌ Payment error: {pay_result['error']}")
        return
    ...
    print("=" * 80)
    print("STEP 3: Polling for image availability")
    print("=" * 80 + "\n")
    ...
    while elapsed < max_wait:
        ...
        if status == "available":
            print(f"\n✅ Image is available! ({elapsed}s elapsed)\n")
            break
        elif status == "pending":
            print(f"⏳ Payment pending... ({elapsed}s elapsed)", end='\r')
        else:
    ...
    if not image_base64_list or not any(image_base64_list):
        print(f"❌ No images in response")
        return
    ...
    for i, base64_data in enumerate(image_base64_list, 1):
        if base64_data:
            ...
            print(f"❌ Image {i}: Failed to retrieve")
